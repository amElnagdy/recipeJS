...
function removePreviousRecipe() {
  ...
  const recipeDetailsContainer = document.querySelector(`#details`);
...
async function doSearch(e) {
  ...
  (0, _videoList.removeVideos)(); //fix videos list keep showing while doing another search
  ...
  removeSpinner(); //Display Recipe

  if (recipe != null) {
    const markup = (0, _recipeDisplay.default)();
    const recipeDetailsMarkup = (0, _recipeDisplay.recipeDetails)(); // document.querySelector(`#app`).insertAdjacentHTML(`beforeend`, markup);

    document.querySelector(`#app`).innerHTML = markup;
    document.querySelector(`#details`).innerHTML = recipeDetailsMarkup; //Display a button to load videos

    (0, _recipeDisplay.loadVideosClick)();
  } // Videos:
